...
export async function getGroupInfo(group_id) {
	let info = await axios.get(`${inforer_api_endpoint}/group/${group_id}`, { timeout: 6000 });
	return info.data.result.group;
}

export async function getScheduleDates(group_id) {
	let dates = await axios.get(`${schedler_api_endpoint}/group/${group_id}/dates`, { timeout: 6000 });
	return dates.data.dates;
}

export async function getGroupSchedule(group_id, date) {
	let schedule = await axios.get(`${schedler_api_endpoint}/group/${group_id}/${date}`, { timeout: 6000 });
