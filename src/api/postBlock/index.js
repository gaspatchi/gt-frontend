...
export async function getPost(post_id) {
	let post = await axios.get(`${wp_api_endpoint}/wp/v2/posts/${post_id}`, { timeout: 3000 });
	post = _.pick(post.data, ["id", "date", "title", "content", "excerpt", "categories"]);
	return post;
}

export async function getlastPosts(category) {
	let posts = await axios.get(`${wp_api_endpoint}/wp/v2/posts`, { timeout: 3000, params: { categories: category, per_page: 4 } });
